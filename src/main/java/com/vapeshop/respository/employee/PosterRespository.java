...
public class PosterRespository {
    ...
    public static Poster getPosterById(String posterId) {
        Poster poster = null;
        try {
            ...
            if (resultSet.next()) {
                 ...
                 String imgUrl=resultSet.getString("url_image");

                 LocalDateTime dateCreate=resultSet.getObject("date_create",LocalDateTime.class);
                 char status=resultSet.getString("status").charAt(0);
            poster.setId(id);
            poster.setImgUrl(imgUrl);
            poster.setDateCreate(dateCreate);
